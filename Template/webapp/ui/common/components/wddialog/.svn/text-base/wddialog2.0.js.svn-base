...
//	}
	...
	this[alertFunName] = function(objs, spantitle, fn) {
		...
		if(objs.headbuttonareaflg) {
			strHtml += " <li class='lititlestyle' style='cursor: move;'><div class='lititledeteilstyle' style='width:"+(objs.width-50)+"px;text-overflow: ellipsis;white-space: nowrap;overflow: hidden;' title='" + objs.title + "' >" + objs.title + "</div><div id='wd-alert-X" + uuid + "' class='lititlectrlstyle' onclick='doalertOk(\"" + uuid + "\")'></div></li>";
		}
...
//		FramitemMove("Framitem", objs);
	...
	this[errorFunName] = function(objs) {
		...
		if(objs.headbuttonareaflg) {
			strHtml += " <li class='lititlestyle' style='cursor: move;'><div class='lititledeteilstyle' style='width:"+(objs.width-50)+"px;text-overflow: ellipsis;white-space: nowrap;overflow: hidden;' title='" + objs.title + "' >" + objs.title + "</div><div id='wd-error-X" + uuid + "' class='lititlectrlstyle' onclick='doerrorOk(\"" + uuid + "\")'></div></li>";
		}
...
//		FramitemMove("Framitem", objs);
	...
	this[successFunName] = function(objs) {
		...
		strHtml = "<ul class='ulstyle' style='" + ulCss + "'>";
		if(objs.headbuttonareaflg) {
			strHtml += " <li class='lititlestyle' style='cursor: move;'><div class='lititledeteilstyle' style='width:"+(objs.width-50)+"px;text-overflow: ellipsis;white-space: nowrap;overflow: hidden;' title='" + objs.title + "' >" + objs.title + "</div><div id='wd-success-X" + uuid + "' class='lititlectrlstyle' onclick='dosuccessOk(\"" + uuid + "\")'></div></li>";
		}
...
//		FramitemMove("Framitem", objs);
	...
	this[confirmFunName] = function(objs) {
		var defaults = {
			...
			showbuttonarealine: false, /////////////////////////////////////new
			innerhtml: null,
			successCallBackFun:null,
			cancelCallBackFun:null
		}
		if(arguments.length > 1){// 支持 旧版本
			if(typeof arguments[0] == "string"){
				defaults.txt = arguments[0];
			}
			if(typeof arguments[2] == "function"){
				defaults.successCallBackFun = arguments[2];
			}
			if(typeof arguments[3] == "function"){
				defaults.cancelCallBackFun = arguments[3];
			}
			objs = defaults;
		}else{
			objs = $.extend(defaults, objs);
		}
		...
		strHtml = "<ul class='ulstyle' style='" + ulCss + "'>";
		if(objs.headbuttonareaflg) {
			strHtml += " <li class='lititlestyle' style='cursor: move;'><div class='lititledeteilstyle' style='width:"+(objs.width-50)+"px;text-overflow: ellipsis;white-space: nowrap;overflow: hidden;' title='" + objs.title + "' >" + objs.title + "</div><div id='wd-confirm-X" + uuid + "' class='lititlectrlstyle' onclick='doconfirmCanle(\"" + uuid + "\")'></div></li>";
		}
		var innerHTML = "";
		if(objs.innerhtml) {
...
//		FramitemMove("Framitem", objs);
	...
	this[dialogFunName] = function(objs) {
		...
		if(objs.positionLeft){
			condialog.style.marginLeft = "-" + parseInt(objs.positionLeft) + "px";
		}else{
		...
		strHtml = "<ul class='ulstyle' style='" + ulCss + "'>";
		if(objs.headbuttonareaflg) {
			strHtml += " <li class='lititlestyle'><div class='lititledeteilstyle' style='width:"+(objs.width-100)+"px;text-overflow: ellipsis;white-space: nowrap;overflow: hidden;padding-left:" + padding + "' title='" + objs.title + "' >" + objs.title + "</div>";
		...
		if(objs.buttonareaflg != false) {
			...
			if(objs.buttons != undefined) {
				...
				strHtml += " <input name='" + uuid + "'  id='wd-condialog-cancel" + uuid + "' class='libuttonokstyle' type='button' value='" + span5 + "' onclick='dodialogCanle" + uuid + "(\"" + uuid + "\")' />";
				strHtml += " <div></li>";
			}
		}
		strHtml += "</ul>";
		...
		this["dodialogOk" + uuid] = function(selfuuid) {
			var _obj = document.activeElement;
			if(document.documentElement.style.overflowY == "hidden") {
				document.documentElement.style.overflowY = "";
			}
			if(objs.successCallBackFun) {
				objs.successCallBackFun.call(this,_obj);
			}
		}
...
function wdGetIframeObj(exceptionFlg) {
	...
	if(typeof(exceptionFlg) == "object"){
		_ = exceptionFlg;
		exceptionFlg = undefined;
	}
	if(exceptionFlg == undefined) {
		subuuid = _.name;
	} else {
